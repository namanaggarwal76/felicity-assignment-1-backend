...
router.post('/:eventId', authMiddleware, checkRole(['user']), async (req, res) => {
  try {
    ...
    if (!registration) {
      return res.status(403).json({ error: 'Only attendees can submit feedback' });
    }
...
router.get('/:eventId', authMiddleware, checkRole(['club']), async (req, res) => {
  try {
    const { eventId } = req.params;
...
router.get('/:eventId/export', authMiddleware, checkRole(['club']), async (req, res) => {
  try {
    const { eventId } = req.params;
...
router.get('/user/can-submit/:eventId', authMiddleware, checkRole(['user']), async (req, res) => {
  try {
    const { eventId } = req.params;
